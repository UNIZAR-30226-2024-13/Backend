...
public class CinquilloDAO {
	...
	public int insert() throws SQLException {
		...
		Cinquillo cinquillo = new Cinquillo(id_nueva_partida, "");
		
		String sql = "INSERT INTO Blackjack(id, escaleras) VALUES (?, ?, ?)"; // Campos de Blackjack
		statemenmt = conn.prepareStatement(sql);

        // Campos de Blackjack
		statemenmt.setInt(1, cinquillo.getId());
		statemenmt.setString(2, cinquillo.getEscaleras());
		statemenmt.execute();  
